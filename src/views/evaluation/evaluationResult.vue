<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="{row}">
          <el-button
            type="primary"
            @click="groupDetails(row)"
          >
            团体详情
          </el-button>
          <el-button
            type="success"
            @click="personDetails(row.id)"
          >
            个人详情
          </el-button>
        </template>
    ...
    >
      ...
      <div>是否预警   ：{{temp.warningLevel}}</div>
      <div
        slot="footer"
        class="dialog-footer"
      >
    ...
    >
      <div class="filter-container">
        ...
        />
        <el-select
          v-model="personlistQuery.department"
          placeholder="所属部门"
          clearable
          class="filter-item"
          style="width: 130px"
        >
          <el-option
            v-for="item in testObjectOptions"
            :key="item.id"
            :label="item.name"
            :value="item.id"
          />
...
<script>
import { fetchList , fetchresult} from '@/api/evaluationresult'
...
export default {
  ...
  methods: {
    ...
    personDetails(planid){
      // this.listLoading = true
      fetchresult(planid).then(response => {
