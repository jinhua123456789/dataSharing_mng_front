...
export default {
  ...
  mounted () {
    ...
    if (this.$route.query.isOneView) {
      // 第三个功能点(查看某个视图的ER图)触发该条件
      const params = {
        params: {
      ...
      .then((res) => {
        console.log(res)
        console.log("this.$route.query.viewName: ", this.$route.query.viewName)
        this.tables = res[0].data;
        this.relations = res[1].data;
        this.draw();
      });
  ...
  methods: {
    getMiddleColumnName(table) {
      ...
      return table.columns[Math.floor(len/2)].columnName;
    },
    draw() {
      ...
      const dataTransform = () => {
        ...
        for (const relation of this.relations) {
          let edge = {
            ...
            'target': relation.to,
            'sourceKey': this.getMiddleColumnName(relation.from),
            'targetKey': this.getMiddleColumnName(relation.to),
