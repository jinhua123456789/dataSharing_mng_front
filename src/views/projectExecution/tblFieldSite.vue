<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input placeholder="站点名称" style="width: 200px;" class="filter-item" v-model="searchValue1" />
    ...
    <el-table :key="tableKey" :data="fakeData" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column align="center" label="站点名称" min-width="80px" fixed>
        <template slot-scope="{row}">
          <el-button type="text">{{row.siteName }}</el-button>
        </template>
      </el-table-column>
    ...
    <div>
      <el-dialog :close-on-click-modal="false" :title="dialogTitle" :visible.sync="dialogProjectVisible" top="0" @close="beforeDialogClose">
        <el-form ref="dataForm" :rules="rules" :model="newItem" label-position="left" label-width="110px" style="width: 680px; margin-left:20px;">
          <el-form-item label="站点编号" prop="siteCode" v-if="isModify">
            <el-input v-model="newItem.siteCode" maxlength="15" placeholder="请输入站点编号" disabled/>
          </el-form-item>
          <el-form-item label="站点编号" prop="siteCode" v-else>
            <el-input v-model="newItem.siteCode" maxlength="15" placeholder="请输入站点编号" />
          </el-form-item>

          <el-form-item label="站点名称" prop="siteName">
            <el-input v-model="newItem.siteName" placeholder="请输入站点名称" />
          </el-form-item>
...
import { getAllTblFieldSite, modifyTblFieldSite, deleteTblFieldSite, addTblFieldSite, searchTblFieldSite, getAllJingLiu, getAlBasins, getDevices, fetchDict, searchDeviceByCodeAndType, locateMinorDrainageBasin, locateJingLiu } from '@/api/projectExcution'
...
export default {
  ...
  data () {
    ...
    return {
      currentUrl:process.env.VUE_APP_BUSINESS_API + 'exportPageInfo/vegDensityCoverageInfo',
      allDeviceOptions: [],
      searchCode: '',
      searchType: '',
      pathToUploadImage,
  ...
  methods: {
    ...
    getData () {
      // if (this.searchState) {
      //   searchTblFieldSite(this.searchValue1, this.searchValue2, this.currentPage, 10).then(res => {
      //     console.log(res)
      //     if (res.code === 200) {
      ...
      this.$refs[dataForm].validate((valid) => {
        if (valid) {
          if (this.isModify) {
            ...
            modifyTblFieldSite(this.newItem).then(res => {
              console.log(res)
              if (res.code === 200) {
            ...
            if ((this.newItem.siteCode === undefined) || (this.newItem.siteCode === "")) {
              ...
              // }
              console.log(this.newItem, 'newItem')
              addTblFieldSite(this.newItem).then(res => {
                console.log(res)
                if (res.code === 200) {
    ...
    deleteItem (scope) {
      ...
      .then(_ => {
        deleteTblFieldSite(this.fakeData[scope.$index].id).then(res => {
          if (res.code === 200) {
