<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input placeholder="采样瓶编号" style="width: 200px;" class="filter-item" v-model="searchValue" />
    ...
    <el-table :key="tableKey" :data="fakeData" border fit highlight-current-row style="width: 100%;">
      <el-table-column align="center" label="采样瓶编号" min-width="80px">
        <template slot-scope="{row}">
          <el-button type="text">{{row.bottleId }}</el-button>
        </template>
      </el-table-column>
      <el-table-column align="center" label="采样瓶体积" min-width="80px">
        <template slot-scope="{row}">{{row.boxVolume }}</template>
      </el-table-column>
    ...
    <div>
      <el-dialog :close-on-click-modal="false" :title="dialogTitle" :visible.sync="dialogProjectVisible">
        <el-form ref="dataForm" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
          <el-form-item label="采样瓶编号" prop="stationcode" v-if="isModify">
            <el-input v-model="newItem.bottleId" disabled/>
          </el-form-item>
          <el-form-item label="采样瓶编号" prop="stationcode" v-else>
            <el-input v-model="newItem.bottleId" />
          </el-form-item>
          
          <el-form-item label="采样瓶体积" prop="stationname">
            <el-input v-model="newItem.boxVolume" />
          </el-form-item>
...
import { getAllSamplingBottle, deleteSamplingBottle, searchSamplingBottle, addSamplingBottle, modifySamplingBottle } from '@/api/projectExcution'
...
export default {
  ...
  methods: {
    ...
    getData () {
      getAllSamplingBottle(this.currentPage, 10).then(res => {
        // console.log(res)
        if (res.code === 200) {
    ...
    onComfirm () {
      if (this.isModify) {
        modifySamplingBottle(this.newItem).then(res => {
          console.log(res)
          if (res.code === 200) {
        ...
        if ((this.newItem.bottleId === undefined) || (this.newItem.bottleId === "")) {
          this.$message.error('采样瓶编号不能为空！');
        } else {
          let tmp = Object.assign({}, this.newItem)
          console.log(this.newItem, 'newItem')
          addSamplingBottle(this.newItem).then(res => {
            console.log(res)
            if (res.code === 200) {
    ...
    deleteItem (scope) {
      ...
      .then(_ => {
        deleteSamplingBottle(this.fakeData[scope.$index].id).then(res => {
          if (res.code === 200) {
    ...
    searchByName () {
      ...
      searchSamplingBottle(this.searchValue, 0, 10).then(res => {
        console.log(res)
        if (res.code === 200) {
          if (res.data) {
