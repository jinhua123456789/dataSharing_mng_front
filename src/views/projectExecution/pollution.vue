<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input placeholder="径流小区编码" style="width: 200px;" class="filter-item" v-model="searchValue1" />
      <el-input placeholder="小流域编码" style="width: 200px;" class="filter-item" v-model="searchValue2" />
    ...
    <el-table :key="tableKey" :data="fakeData" border fit highlight-current-row style="width: 100%;">
      <el-table-column align="center" label="测点编号" min-width="80px" fixed>
        <template slot-scope="{row}">
          <span>{{row.measurePointNumber}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="测点名称" min-width="80px" fixed>
        <template slot-scope="{row}">
          <el-button type="text">{{row.measurePointName }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="土地利用" min-width="100px">
        <template slot-scope="{row}">
          <span>{{row.ultilization}}</span>
        </template>
      </el-table-column>

      <!-- <el-table-column align="center" label="测量点类型" min-width="100px">
        <template slot-scope="{row}">
          <span>{{getTypeName(row.measurePointType)}}</span>
        </template>
      </el-table-column> -->
...
<!--      <el-table-column align="center" label="测量方法" min-width="80px">-->
<!--        <template slot-scope="{row}">-->
<!--          <span>{{row.measureMethod}}</span>-->
<!--        </template>-->
<!--      </el-table-column>-->

      <el-table-column align="center" label="所属径流小区名称及编号" min-width="120px">
        <template slot-scope="{row}">
      ...
      <el-table-column align="center" label="所属小流域名称及编号" min-width="120px">
        <template slot-scope="{row}">
    ...
    <div>
      <el-dialog :close-on-click-modal="false" :title="dialogTitle" :visible.sync="dialogProjectVisible" top="0" @close="beforeDialogClose">
        <el-form ref="dataForm" :rules="rules" :model="newItem" label-position="left" label-width="110px" style="width: 680px; margin-left:20px;">
          <el-form-item label="站点编号" prop="measurePointNumber" v-if="isModify">
            <el-input v-model="newItem.measurePointNumber" maxlength="15" placeholder="请输入站点编号" disabled/>
          </el-form-item>
          <el-form-item label="站点编号" prop="measurePointNumber" v-else>
            <el-input v-model="newItem.measurePointNumber" maxlength="15" placeholder="请输入站点编号" />
          </el-form-item>

          <el-form-item label="站点名称" prop="measurePointName">
            <el-input v-model="newItem.measurePointName" placeholder="请输入站点名称" />
          </el-form-item>
          ...
          <!-- <el-form-item label="测量点类型" prop="measurePointType">
            <el-select v-model="newItem.measurePointType" placeholder="请选择">
          ...
          <el-form-item label="测量方法" prop="measureMethod">
            <el-input v-model="newItem.measureMethod" />
          </el-form-item>
          ...
          <!-- <el-form-item label="安装时间" prop="">
            <el-date-picker type="date" placeholder="选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd" @input="getdate" v-model="newItem.date1" style="width: 100%;"></el-date-picker>
          </el-form-item> -->
          ...
          <el-form-item label="说明" prop="">
            <el-input type="textarea" rows="5" v-model="newItem.details" />
          </el-form-item>
...
import { getAllPollution, modifyPollution, deletePollution, addPollution, searchPollution, getAllJingLiu, getAlBasins, getDevices, fetchDict, searchDeviceByCodeAndType, locateMinorDrainageBasin, locateJingLiu } from '@/api/projectExcution'
...
export default {
  ...
  methods: {
    ...
    getData () {
      if (this.searchState) {
        searchPollution(this.searchValue1, this.searchValue2, this.currentPage, 10).then(res => {
          console.log(res)
          if (res.code === 200) {
      ...
      getAllPollution(this.currentPage, 10).then(res => {
        // console.log(res)
        if (res.code === 200) {
          this.totalElements = res.data.totalElements
          this.fakeData = res.data.content
      ...
      this.$refs[dataForm].validate((valid) => {
        if (valid) {
          if (this.isModify) {
            ...
            modifyPollution(this.newItem).then(res => {
              console.log(res)
              if (res.code === 200) {
            ...
            if ((this.newItem.measurePointNumber === undefined) || (this.newItem.measurePointNumber === "")) {
              ...
              // }
              console.log(this.newItem, 'newItem')
              addPollution(this.newItem).then(res => {
                console.log(res)
                if (res.code === 200) {
    ...
    deleteItem (scope) {
      ...
      .then(_ => {
        deletePollution(this.fakeData[scope.$index].id).then(res => {
          if (res.code === 200) {
    ...
    searchByName () {
      this.currentPage = 0
      this.searchState = (!this.searchValue1.length && !this.searchValue2.length) ? false : true
      this.getData()
    }
  }
}
