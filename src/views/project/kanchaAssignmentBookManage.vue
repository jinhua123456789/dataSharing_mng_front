<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      />
      <el-input
        v-model="listQuery.assignmentId"
        placeholder="任务书编号"
        style="width: 200px;"
        @keyup.enter.native="handleFilter"
      />
    ...
    >
      ...
      <el-table-column label="任务书编号" min-width="50px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.assignmentId.number }}</span>
        </template>
      </el-table-column>
      <el-table-column label="下达任务书时间" min-width="50px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.assignmentTime| parseTime('2019-{m}-{d}') }}</span>
      ...
      >
        <template slot-scope="{row}">
          <el-button v-if="row.status!='已确认'" size="mini" type="success" @click="handleModifyStatus(row,'已确认')">
            确认
          </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="探矿权人" prop="trueName">
          <el-input v-model="temp.trueName" style="width:410px" type="text" placeholder="所属单位" />
        </el-form-item>
        <el-form-item label="勘察单位" prop="trueName">
